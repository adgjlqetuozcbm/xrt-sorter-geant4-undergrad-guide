    ...
    for seed in SHUFFLE_SEEDS:
        ...
        for eval_split, profile, eval_frame in eval_items:
            ...
            summary, _, _ = evaluate_estimator(
                track="v7_twenty_material_shuffled_label_null",
                model=model,
                estimator=estimator,
...
def write_report(output_dir: Path, gate: dict[str, Any], best: pd.DataFrame, controls: pd.DataFrame, shuffled: pd.DataFrame) -> None:
    ...
    lines.append("")
    (output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_report.md").write_text(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train/evaluate v8A v7 twenty-material scalability scout models.")
    ...
    for eval_split, profile, eval_frame in eval_items:
        for model in models:
            summary, decisions, _ = evaluate_estimator(
                track="v7_twenty_material_scalability_scout",
                model=model,
    ...
    summary.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_summary.csv", index=False)
    best.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_best_main.csv", index=False)
    controls.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_controls.csv", index=False)
    decisions_all = pd.concat(decision_frames, ignore_index=True) if decision_frames else pd.DataFrame()
    if not decisions_all.empty:
        decisions_all.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_decisions.csv", index=False)
        confusions = confusion_rows(decisions_all, labels)
        if not confusions.empty:
            confusions.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_confusions.csv", index=False)
    ...
    shuffled_raw.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_shuffled_raw.csv", index=False)
    shuffled_summary.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_shuffled_summary.csv", index=False)
    ...
    gate = {
        "generated_by": "analysis/train_v8a_multiclass_context_v7_twenty_material_scalability_scout.py",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "development_only": True,
        "claim_scope": "development-only twenty-material perturbation-augmented scalability scout; not hardware validation or product accuracy",
        "gate_passed": gate_passed,
        "decision": "v7_twenty_material_scalability_scout_gate_passed" if gate_passed else "stop_v7_twenty_material_scalability_scout_gate",
    ...
    write_json(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_gate.json", json_clean(gate))
