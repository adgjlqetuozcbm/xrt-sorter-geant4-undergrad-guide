...
def write_report(output_dir: Path, gate: dict[str, Any]) -> None:
    lines = [
        "# v8A v7 twenty-material scalability scout training-data audit",
        "",
    ...
    lines.append("")
    (output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_training_data_final_report.md").write_text(
        "\n".join(lines), encoding="utf-8", newline="\n"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Final-audit v8A v7 twenty-material scalability scout feature table before diagnostics.")
    parser.add_argument("--project-root", default=Path(__file__).resolve().parents[1])
    parser.add_argument("--config", default="analysis/configs/v8a_multiclass_context_v7_twenty_material_scalability_scout_config.json")
    parser.add_argument("--input-dir", required=True)
    parser.add_argument("--matrix-preflight-gate", default="results/accuracy_v3/v8a_multiclass_context_v7_twenty_material_scalability_scout_preflight/v8a_multiclass_context_v7_twenty_material_scalability_scout_preflight_gate.json")
    ...
    split_profile_counts.to_csv(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_split_profile_counts.csv", index=False)
    ...
    warnings.append("source_off_control_not_required_for_v7_scalability_scout_but_total_count_lineage_and_shuffled_controls_are_required_in_model_gate")

    gate_passed = not stop_reasons
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v7_twenty_material_scalability_scout_training_data_final.py",
        ...
        "decision": "v7_twenty_material_scalability_scout_diagnostics_unlocked" if gate_passed else "stop_v7_twenty_material_scalability_scout_training_data",
    ...
    write_json(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_training_data_final_gate.json", json_clean(gate))
