...
THRESHOLDS = {
    ...
    "off_diagonal_main_errors_max": 0,
...
def best_main_off_diagonal_errors(confusions: pd.DataFrame, best_main: pd.DataFrame) -> int:
    ...
    off = merged[merged["actual"].astype(str) != merged["predicted"].astype(str)].copy()
    return int(off["count"].sum()) if "count" in off.columns else int(len(off))


def write_report(output_dir: Path, gate: dict[str, Any]) -> None:
    lines = [
        "# v8A v6 skeptical shortcut audit",
        "",
        ...
        f"- Off-diagonal main-model errors: `{gate['off_diagonal_main_errors_count']}`",
    ...
    lines.append("")
    (output_dir / "v8a_multiclass_context_v6_skeptical_shortcut_report.md").write_text(
        "\n".join(lines), encoding="utf-8", newline="\n"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Skeptical shortcut audit for v8A v6 physical robustness results.")
    ...
    model_gate = load_json(model_dir / "v8a_multiclass_context_v6_physical_robust_gate.json")
    best_main = pd.read_csv(model_dir / "v8a_multiclass_context_v6_physical_robust_best_main.csv")
    confusions_path = model_dir / "v8a_multiclass_context_v6_physical_robust_confusions.csv"
    ...
    if not overlap["overlap"].empty:
        overlap["overlap"].to_csv(output_dir / "v8a_multiclass_context_v6_cross_split_main_feature_hash_overlap.csv", index=False)

    lineage_like_main = [col for col in main_cols if any(token in col.lower() for token in LEAK_TOKENS)]
    off_diagonal_count = best_main_off_diagonal_errors(confusions, best_main)
    ...
    if off_diagonal_count > THRESHOLDS["off_diagonal_main_errors_max"]:
        stop_reasons.append("off_diagonal_main_model_errors_detected")
    ...
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v6_skeptical_shortcut.py",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "development_only": True,
        "claim_scope": "skeptical development audit for v6 physical robustness; not hardware validation or product accuracy",
        "gate_passed": gate_passed,
        "decision": "v6_skeptical_shortcut_audit_passed" if gate_passed else "stop_v6_skeptical_shortcut_audit",
    ...
    write_json(output_dir / "v8a_multiclass_context_v6_skeptical_shortcut_gate.json", json_clean(gate))
    write_report(output_dir, gate)
    print(
        "decision={decision} gate_passed={passed} overlap={overlap_count} offdiag={offdiag}".format(
            decision=gate["decision"],
            passed=str(gate_passed).lower(),
            overlap_count=gate["cross_split_exact_main_feature_hash_overlap_count"],
            offdiag=gate["off_diagonal_main_errors_count"],
        )
    )
