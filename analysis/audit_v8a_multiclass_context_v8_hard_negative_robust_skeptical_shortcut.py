...
def write_report(output_dir: Path, gate: dict[str, Any]) -> None:
    lines = [
        "# v8A v8 twenty-material hard-negative robustness skeptical shortcut audit",
        "",
    ...
    lines.append("")
    (output_dir / "v8a_multiclass_context_v8_hard_negative_robust_skeptical_shortcut_report.md").write_text(
        "\n".join(lines), encoding="utf-8", newline="\n"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Skeptical shortcut audit for v8A v8 twenty-material hard-negative robustness results.")
    ...
    model_gate = load_json(model_dir / "v8a_multiclass_context_v8_hard_negative_robust_gate.json")
    best_main = pd.read_csv(model_dir / "v8a_multiclass_context_v8_hard_negative_robust_best_main.csv")
    confusions_path = model_dir / "v8a_multiclass_context_v8_hard_negative_robust_confusions.csv"
    ...
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v8_hard_negative_robust_skeptical_shortcut.py",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "development_only": True,
        "claim_scope": "skeptical development audit for v8 hard-negative robustness; not hardware validation or product accuracy",
        "gate_passed": gate_passed,
        "decision": "v8_hard_negative_robust_skeptical_shortcut_audit_passed" if gate_passed else "stop_v8_hard_negative_robust_skeptical_shortcut_audit",
    ...
    write_json(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_skeptical_shortcut_gate.json", json_clean(gate))
