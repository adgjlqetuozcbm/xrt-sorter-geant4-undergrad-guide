...
PROFILE = "v8a_multiclass_context_v4_count_overlap_50kev_t60_t120"
PEAK_MANIFEST = "source_models/config/diffraction_peak_tables/ten_material_powder_peaks_cif_or_literature_v8a_manifest.json"
EVENT_TO_FEATURE_DIR = "results/accuracy_v3/v8a_multiclass_context_v4_count_overlap_event_to_feature"
CLEANLINESS_DIR = "results/accuracy_v3/v8a_multiclass_context_v4_count_overlap_cleanliness_design"
SHORTCUT_DIR = "results/accuracy_v3/v8a_multiclass_context_v4_count_overlap_feature_shortcut_structure"
FINAL_AUDIT_DIR = "results/accuracy_v3/v8a_multiclass_context_v4_count_overlap_training_data_final_audit"
MODEL_DIR = "results/accuracy_v3/v8a_multiclass_context_v4_count_overlap_development_model"
EXPECTED_ROWS = 3200
...
def wait_for_geant4(project_root: Path, python_exe: str, poll_seconds: int) -> None:
    while True:
        ...
        if current["completed"] >= EXPECTED_ROWS and current["pending"] == 0:
            print(f"[{timestamp()}] v4 count-overlap Geant4 complete: {current}", flush=True)
            return
        if not runner_active():
            print(f"[{timestamp()}] v4 count-overlap runner is not active; resuming profile {PROFILE}.", flush=True)
...
def postprocess(project_root: Path, python_exe: str, overwrite: bool) -> None:
    ...
    commands = [
        ...
        [
            ...
            "--input-dir",
            EVENT_TO_FEATURE_DIR,
            "--matrix-preflight-gate",
            "results/accuracy_v3/v8a_multiclass_context_v4_count_overlap_preflight/v8a_multiclass_context_v4_count_overlap_preflight_gate.json",
            "--output-dir",
            FINAL_AUDIT_DIR,
            *overwrite_flag,
        ],
        [
            ...
            "--output-dir",
            MODEL_DIR,
            "--count-balance-strategy",
            "sliding_window_0p050",
            *overwrite_flag,
        ],
    ]
    for cmd in commands:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Wait for v8A v4 count-overlap Geant4, then run downstream diagnostics.")
    ...
    print(f"[{timestamp()}] v8A v4 count-overlap pipeline complete", flush=True)
