...
def split_seed_blocks(config: dict[str, Any], split: str) -> list[tuple[str, int]]:
    plan = config["split_seed_plan"][split]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the v8A full-10 count-overlap training-candidate matrix.")
    ...
    if config.get("status") != "development_training_preregistration":
        raise RuntimeError("Count-overlap config must remain development_training_preregistration.")
    ...
    row_index = 0
    source_energies = [float(item) for item in config["source_energies_kev"]]
    if len(source_energies) != 1:
        raise RuntimeError("v4 count-overlap matrix is intentionally limited to one selected energy.")
    source_energy = source_energies[0]
    energy_tag = f"{source_energy:g}kev".replace(".", "p")
    source_id = f"v8a_full10_count_overlap_{energy_tag}"
                ...
                for pose_index in config["pose_indices"]:
                    ...
                    for count_index, count_config in enumerate(config["count_target_bins"]):
                        ...
                        context_cell_id = (
                            ...
                            f"c{count_target_bin}"
                        )
                        nuisance_cell_id = (
                            f"{profile}|{split}|{config['clean_matrix_origin']}|{config['source_family']}|"
                            f"e{source_energy:g}|t{thickness:g}|p{pose_index}|c{count_target_bin}|{seed_block}"
                        )
                        for material in materials:
                            ...
                            write_config(
                                project_root / config_rel,
                                {
                                    ...
                                    "research_route": "v8a_multiclass_context_count_overlap",
                                    "prediction_stage": "full10_count_overlap_training_candidate",
                                    "run_role": "material",
    ...
    manifest = {
        "generated_by": "analysis/generate_v8a_multiclass_context_v4_count_overlap_matrix.py",
        "config": args.config,
        "profile": profile,
        "development_only": True,
        "shadow_or_final_used": False,
