...
def error_rows(model_dir: Path) -> pd.DataFrame:
    decisions_path = model_dir / "v8a_multiclass_context_v8_hard_negative_robust_decisions.csv"
    ...
    return decisions[
        ...
        & decisions["method"].astype(str).isin(["LogisticMulticlassMain", "ExtraTreesMulticlassMain"])
    ].copy()
...
def write_report(output_dir: Path, gate: dict[str, Any], nearest: pd.DataFrame, pair_distances: pd.DataFrame, peak_summary: pd.DataFrame) -> None:
        ...
        compact_table(
            peak_summary[
                peak_summary["physical_perturbation_profile"].astype(str).isin(
                    [
                        "combined_train_high_neg",
                        "combined_train_high_pos",
                        "hard_negative_peak_shift_train",
                        "combined_stress_high",
                        "hard_negative_stress_alt",
                    ]
                )
    ...
    (output_dir / "v8a_multiclass_context_v8_hard_negative_robust_diagnostics_report.md").write_text(
        "\n".join(lines), encoding="utf-8", newline="\n"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Diagnose v8 Hematite/Magnetite/Ilmenite hard-negative behavior.")
    ...
    parser.add_argument("--output-dir", default="results/accuracy_v3/v8a_multiclass_context_v8_hard_negative_robust_hard_negative_diagnostics")
    ...
    peak_summary.to_csv(output_dir / "v8a_multiclass_context_v8_peak_group_sums_by_profile_material.csv", index=False)
    ...
    nearest_parts = []
    for query_profile in [
        "combined_stress_high",
        "hard_negative_stress_alt",
        "calibration_shift_stress_neg",
        "resolution_blur_stress_high",
    ]:
        ...
        for train_profile in [
            "combined_train_high_neg",
            "combined_train_high_pos",
            "hard_negative_peak_shift_train",
            "combined_train_moderate",
            "resolution_blur_train_high",
        ]:
            nearest_parts.append(
                nearest_train_centroids(
                    centroids,
    ...
    nearest.to_csv(output_dir / "v8a_multiclass_context_v8_hematite_stress_nearest_train_centroids.csv", index=False)

    pair_distances = pd.concat(
        [
            focus_pair_distances(centroids, profile)
            for profile in [
                "combined_stress_high",
                "hard_negative_stress_alt",
                "calibration_shift_stress_neg",
    ...
    pair_distances.to_csv(output_dir / "v8a_multiclass_context_v8_stress_focus_pair_distances.csv", index=False)

    errors = error_rows(model_dir)
    errors.to_csv(output_dir / "v8a_multiclass_context_v8_hm_ilmenite_error_rows.csv", index=False)

    model_gate_path = model_dir / "v8a_multiclass_context_v8_hard_negative_robust_gate.json"
    ...
    hematite_ilmenite_error_count = int(
        len(errors[
            ...
            & ~errors["physical_perturbation_profile"].astype(str).eq("__overall__")
        ])
    ) if not errors.empty else 0
    nearest_top = (
        nearest[nearest["train_profile_filter"].astype(str).eq("__all_train_profiles__")]
        .groupby("query_profile", sort=True)
        .head(3)[["query_profile", "candidate_profile", "candidate_material", "euclidean_distance_zscore"]]
        .to_dict("records")
        if not nearest.empty
        else []
    )
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v8_hard_negative_robust_diagnostics.py",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "development_only": True,
        "diagnostic_completed": True,
        "decision": "v8_hard_negative_diagnostic_completed_next_step_feature_or_sampling_rework",
        "main_finding": "v8 targeted hard-negative training improves the v7 combined-stress collapse but still leaves Hematite close to Ilmenite under held-out hard stress profiles",
        ...
        "hematite_to_ilmenite_error_rows_excluding_overall": hematite_ilmenite_error_count,
        "hematite_to_ilmenite_unique_samples_excluding_overall": int(
            errors[
                errors["material"].astype(str).eq("Hematite")
                & errors["prediction"].astype(str).eq("Ilmenite")
                & ~errors["physical_perturbation_profile"].astype(str).eq("__overall__")
        ...
        if not errors.empty and "sample_id" in errors.columns
        else 0,
        "nearest_train_centroids_top3_for_hematite_stress_profiles": nearest_top,
        "outputs": {
            "peak_group_sums": "v8a_multiclass_context_v8_peak_group_sums_by_profile_material.csv",
            "nearest_train_centroids": "v8a_multiclass_context_v8_hematite_stress_nearest_train_centroids.csv",
            "focus_pair_distances": "v8a_multiclass_context_v8_stress_focus_pair_distances.csv",
            "hm_ilmenite_error_rows": "v8a_multiclass_context_v8_hm_ilmenite_error_rows.csv",
        },
        "software": {"python": platform.python_version(), "pandas": pd.__version__, "numpy": np.__version__},
    }
    write_json(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_diagnostics_gate.json", json_clean(gate))
