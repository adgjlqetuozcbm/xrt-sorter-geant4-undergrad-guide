...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preflight v8A v8 twenty-material hard-negative robustness matrix.")
    ...
    parser.add_argument("--output-dir", default="results/accuracy_v3/v8a_multiclass_context_v8_hard_negative_robust_preflight")
    ...
    if config.get("status") != "development_twenty_material_hard_negative_preregistration":
        stop_reasons.append("config_status_not_development_twenty_material_hard_negative_preregistration")
    ...
    if len(profiles_by_split.get("train", [])) < 10:
        stop_reasons.append("train_split_not_hard_negative_augmented_enough")
    ...
    warnings.append("v8_is_a_development_hard_negative_followup_using_approximate_geant4_material_definitions_and_rruff_reference_peaks")

    gate_passed = not stop_reasons
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v8_hard_negative_robust_preflight.py",
        ...
        "decision": "v8_hard_negative_robust_preflight_passed_ready_for_development_geant4" if gate_passed else "stop_v8_hard_negative_robust_preflight",
        "profile": profile,
        "row_count": len(rows),
    ...
    write_json(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_preflight_gate.json", gate)
