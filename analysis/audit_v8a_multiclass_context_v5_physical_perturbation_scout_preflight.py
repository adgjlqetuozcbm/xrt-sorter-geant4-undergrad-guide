...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preflight v8A v5 physical perturbation scout matrix.")
    ...
    parser.add_argument("--output-dir", default="results/accuracy_v3/v8a_multiclass_context_v5_physical_perturbation_scout_preflight")
    ...
    if config.get("status") != "development_physical_perturbation_scout_preregistration":
        stop_reasons.append("config_status_not_development_physical_perturbation_scout_preregistration")
    ...
    if "combined_moderate" not in profiles_by_split.get("stress_holdout", []):
        stop_reasons.append("stress_holdout_missing_combined_moderate")
    ...
    warnings.append("v5_scout_train_nominal_only_validation_and_stress_include_profile_shift")

    gate_passed = not stop_reasons
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v5_physical_perturbation_scout_preflight.py",
        ...
        "decision": "v5_physical_perturbation_scout_preflight_passed_ready_for_development_geant4" if gate_passed else "stop_v5_physical_perturbation_scout_preflight",
        "profile": profile,
        "row_count": len(rows),
    ...
    write_json(output_dir / "v8a_multiclass_context_v5_physical_perturbation_scout_preflight_gate.json", gate)
