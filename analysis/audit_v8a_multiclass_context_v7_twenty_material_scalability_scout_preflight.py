...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preflight v8A v7 twenty-material scalability scout matrix.")
    ...
    parser.add_argument("--output-dir", default="results/accuracy_v3/v8a_multiclass_context_v7_twenty_material_scalability_scout_preflight")
    ...
    if config.get("status") != "development_twenty_material_scalability_scout_preregistration":
        stop_reasons.append("config_status_not_development_twenty_material_scalability_scout_preregistration")
    ...
    if len(materials) != 20:
        stop_reasons.append(f"material_count_not_20:{len(materials)}")
    ...
    if bad_cells:
        stop_reasons.append(f"context_cells_not_full20_or_cross_split:{bad_cells}")
    ...
    warnings.append("v7_is_a_development_scalability_scout_using_approximate_geant4_material_definitions_and_rruff_reference_peaks")

    gate_passed = not stop_reasons
    gate = {
        "generated_by": "analysis/audit_v8a_multiclass_context_v7_twenty_material_scalability_scout_preflight.py",
        ...
        "decision": "v7_twenty_material_scalability_scout_preflight_passed_ready_for_development_geant4" if gate_passed else "stop_v7_twenty_material_scalability_scout_preflight",
        "profile": profile,
        "row_count": len(rows),
    ...
    write_json(output_dir / "v8a_multiclass_context_v7_twenty_material_scalability_scout_preflight_gate.json", gate)
