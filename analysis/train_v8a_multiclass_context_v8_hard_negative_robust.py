...
SHUFFLE_SEEDS = list(range(77001, 77011))
MAIN_METHODS = ("LogisticMulticlassMain", "ExtraTreesMulticlassMain")
THRESHOLDS = {
    "validation_worst_profile_hm_min_recall_min": 0.80,
    "stress_worst_profile_hm_min_recall_min": 0.70,
    "validation_worst_profile_macro_f1_min": 0.45,
    "stress_worst_profile_macro_f1_min": 0.40,
    "validation_overall_hm_min_recall_min": 0.85,
    ...
    "stress_hematite_to_ilmenite_main_errors_max": 1,
}
...
def fit_shuffled_estimator(model: dict[str, Any], train: pd.DataFrame, shuffle_seed: int) -> Any | None:
...
def markdown_table(frame: pd.DataFrame, columns: list[str], limit: int = 64) -> str:
    if frame.empty:
        return ""
...
def write_report(output_dir: Path, gate: dict[str, Any], best: pd.DataFrame, controls: pd.DataFrame, shuffled: pd.DataFrame) -> None:
    lines = [
        ...
        f"- Worst validation profile macro-F1: `{gate['validation_worst_profile_macro_f1']:.4f}`",
        f"- Worst stress profile macro-F1: `{gate['stress_worst_profile_macro_f1']:.4f}`",
        ...
        "",
        markdown_table(
            shuffled.sort_values(["eval_split", "physical_perturbation_profile", "method"]).head(32),
    ...
    lines.append("")
    (output_dir / "v8a_multiclass_context_v8_hard_negative_robust_report.md").write_text(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train/evaluate v8A v8 twenty-material hard-negative robustness models.")
    ...
    summary.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_summary.csv", index=False)
    best.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_best_main.csv", index=False)
    controls.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_controls.csv", index=False)
    decisions_all = pd.concat(decision_frames, ignore_index=True) if decision_frames else pd.DataFrame()
    if not decisions_all.empty:
        decisions_all.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_decisions.csv", index=False)
        confusions = confusion_rows(decisions_all, labels)
        if not confusions.empty:
            confusions.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_confusions.csv", index=False)
    ...
    shuffled_raw.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_shuffled_raw.csv", index=False)
    shuffled_summary.to_csv(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_shuffled_summary.csv", index=False)
    ...
    stress_h_to_i_errors = 0
    if not decisions_all.empty:
        stress_h_to_i = decisions_all[
            decisions_all["method"].astype(str).isin(MAIN_METHODS)
            & decisions_all["split"].astype(str).eq("stress_holdout")
            & decisions_all["material"].astype(str).eq("Hematite")
            & decisions_all["prediction"].astype(str).eq("Ilmenite")
        ]
        stress_h_to_i_errors = int(len(stress_h_to_i))
    ...
    if stress_h_to_i_errors > THRESHOLDS["stress_hematite_to_ilmenite_main_errors_max"]:
        stop_reasons.append("stress_hematite_to_ilmenite_main_errors_above_threshold")

    gate_passed = not stop_reasons
    gate = {
        "generated_by": "analysis/train_v8a_multiclass_context_v8_hard_negative_robust.py",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "development_only": True,
        "claim_scope": "development-only twenty-material hard-negative robustness follow-up; not hardware validation or product accuracy",
        "gate_passed": gate_passed,
        "decision": "v8_hard_negative_robust_gate_passed" if gate_passed else "stop_v8_hard_negative_robust_gate",
        ...
        "stress_hematite_to_ilmenite_main_errors": stress_h_to_i_errors,
        "thresholds": THRESHOLDS,
        "stop_reasons": stop_reasons,
        "software": {"python": platform.python_version(), "pandas": pd.__version__, "numpy": np.__version__},
    }
    write_json(output_dir / "v8a_multiclass_context_v8_hard_negative_robust_gate.json", json_clean(gate))
    write_report(output_dir, gate, best, controls, shuffled_summary)
    print(
        "decision={decision} gate_passed={passed} val_worst_hm={val:.4f} stress_worst_hm={stress:.4f} val_macro={val_macro:.4f} stress_macro={stress_macro:.4f}".format(
            decision=gate["decision"],
            passed=str(gate_passed).lower(),
            val=validation_worst_hm,
            stress=stress_worst_hm,
            val_macro=validation_worst_macro,
            stress_macro=stress_worst_macro,
        )
    )
